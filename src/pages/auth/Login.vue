...
<template>
  <div class="h-screen w-screen bg-gray-100 flex items-center justify-center p-4 overflow-hidden">
    <div class="max-w-7xl w-full bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row h-full max-h-[700px]">
      ...
      <div class="w-full md:w-2/5 p-8 flex flex-col justify-between h-full overflow-y-auto custom-scrollbar">
        <div class="mb-6">
          ...
          <form @submit.prevent="handleLogin" class="space-y-4">
            <div>
              <label class="block text-xs font-medium text-gray-950 ml-1 mb-1.5">Tài khoản</label>
              <input
                v-model="email"
                type="text"
            ...
            <div class="flex items-center justify-between text-xs pt-1">
              <label class="flex items-center cursor-pointer gap-2">
                <div class="relative">
                  ...
                  <div
                    :class="rememberMe ? 'bg-blue-600' : 'bg-gray-300'"
                    class="block w-8 h-4 rounded-full transition-colors"
                  ></div>
                  <div
                    :class="rememberMe ? 'translate-x-4' : 'translate-x-1'"
                    class="absolute left-0 top-1 bg-white w-2 h-2 rounded-full transition-transform"
                  ></div>
                </div>
                <span class="text-gray-600 font-medium">Nhớ tài khoản</span>
              </label>
            ...
            <button
              :disabled="isLoading"
              class="cursor-pointer w-full bg-[#007bff] hover:bg-blue-700 text-white font-semibold py-3 rounded-lg transition-colors shadow-lg shadow-blue-200 mt-2 disabled:opacity-70 disabled:cursor-not-allowed flex justify-center items-center gap-2"
            >
            ...
            <div class="relative flex items-center py-2">
              ...
              <span class="flex-shrink mx-4 text-gray-400 text-xs">Or sign in with Google</span>
              <div class="flex-grow border-t border-gray-200"></div>
            </div>

            <button
              type="button"
              class="w-full flex items-center justify-center gap-3 bg-[#333] hover:bg-black text-white font-medium py-3 rounded-lg transition-all"
            >
              <img
        ...
        <div class="flex justify-between items-center text-[10px] text-gray-400 mt-4">
...
<style scoped>
/* Optional: Tùy chỉnh thanh cuộn nếu màn hình quá nhỏ buộc phải cuộn */
